...
export default function AdminLogin() {
  ...
  useEffect(() => {
    if (!authLoading && !roleLoading && user) {
      if (hasAnyAdminRole) navigate("/admin/tenants", { replace: true });
  ...
  const handleSignOut = async () => { await supabase.auth.signOut(); setAccessDenied(false); };

  if (authLoading || roleLoading) return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-navy"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
  );
        ...
        {accessDenied ? (
          <Card className="border-destructive/30 bg-secondary/40 backdrop-blur-sm">
            <CardHeader className="text-center">
              <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-destructive/20"><AlertTriangle className="h-6 w-6 text-destructive" /></div>
              <CardTitle className="font-display text-xl text-secondary-foreground">Access Denied</CardTitle>
              <CardDescription className="text-secondary-foreground/50">Your account does not have administrator privileges.</CardDescription>
            </CardHeader>
            <CardContent><Button onClick={handleSignOut} variant="outline" className="w-full">Sign Out & Try Another Account</Button></CardContent>
          </Card>
        ) : (
          <Card className="border-secondary/20 bg-secondary/40 backdrop-blur-sm">
            <CardHeader className="text-center">
              <CardTitle className="font-display text-2xl text-secondary-foreground">Admin Login</CardTitle>
              <CardDescription className="text-secondary-foreground/50">Sign in with your administrator credentials</CardDescription>
            </CardHeader>
            <CardContent>
