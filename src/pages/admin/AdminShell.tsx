...
export default function AdminShell() {
  ...
  useEffect(() => {
    if (!authLoading && !adminRole.loading) {
      if (!user || !adminRole.hasAnyAdminRole) navigate("/admin", { replace: true });
    }
  }, [authLoading, adminRole.loading, user, adminRole.hasAnyAdminRole, navigate]);

  if (authLoading || adminRole.loading) return (
    <div className="flex min-h-screen items-center justify-center bg-background"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
  );
          ...
          {visibleNav.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} className={cn(
                ...
                active ? "bg-sidebar-primary/15 text-sidebar-primary font-semibold shadow-sm border border-sidebar-primary/20" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-primary"
              )}>
