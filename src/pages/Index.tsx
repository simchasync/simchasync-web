    ...
    <div className="min-h-screen bg-gradient-navy text-secondary-foreground">
      ...
      <section className="relative overflow-hidden py-20 md:py-32">
        ...
        <div className="container relative">
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="mx-auto max-w-3xl text-center">
            ...
            <p className="mx-auto mb-10 max-w-2xl text-lg text-secondary-foreground/60 md:text-xl">{l.hero.subtitle}</p>
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              <Link to="/auth?mode=signup">
                <Button size="lg" className="bg-gradient-gold shadow-gold text-primary-foreground font-semibold text-lg px-8 py-6 hover:opacity-90">
                  {l.hero.cta}<ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </Link>
              <a href="#features">
                <Button size="lg" variant="outline" className="border-foreground/30 text-foreground hover:border-primary hover:text-primary text-lg px-8 py-6">{l.hero.ctaSecondary}</Button>
              </a>
            </div>
          </motion.div>
        </div>
      ...
      <section id="features" className="py-20 md:py-28">
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} className="mb-16 text-center">
            <motion.h2 variants={fadeUp} custom={0} className="mb-4 font-display text-3xl font-bold text-secondary-foreground md:text-5xl">{l.features.title}</motion.h2>
            <motion.p variants={fadeUp} custom={1} className="text-lg text-secondary-foreground/50">{l.features.subtitle}</motion.p>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <motion.div key={i} variants={fadeUp} custom={i + 2}>
                  <Card className="group border-secondary/20 bg-secondary/40 backdrop-blur-sm transition-all hover:border-primary/30 hover:shadow-gold/10 hover:shadow-lg h-full">
                    <CardContent className="p-6">
                      <div className="mb-4 inline-flex rounded-xl bg-primary/10 p-3"><Icon className="h-6 w-6 text-primary" /></div>
        ...
        <div className="container">
          ...
          <div className="mx-auto grid max-w-4xl gap-8 md:grid-cols-2">
            {l.pricing.plans.map((plan, i) => (
              <Card key={i} className={`relative border-secondary/20 bg-secondary/40 backdrop-blur-sm transition-all ${plan.popular ? "border-primary/50 shadow-gold" : ""}`}>
                {plan.popular && (
                ...
                <CardContent className="p-8">
                      ...
                      return (
                        <li key={j} className={`flex items-start gap-2 text-sm ${isComingSoon ? "text-secondary-foreground/40" : "text-secondary-foreground/70"}`}>
                          ...
                          <span className="flex items-center gap-1.5 flex-wrap">
                            {label}
                            {isComingSoon && (<Badge variant="secondary" className="text-[10px] px-1.5 py-0 font-normal bg-secondary/60 text-secondary-foreground/50 border-0">Coming Soon</Badge>)}
                          </span>
                        </li>
                      );
                    })}
                  ...
                  <Link to="/auth?mode=signup">
                    <Button className={`w-full font-semibold ${plan.popular ? "bg-gradient-gold shadow-gold text-primary-foreground hover:opacity-90" : "border-primary/30 text-primary hover:bg-primary/10"}`} variant={plan.popular ? "default" : "outline"}>{plan.cta}</Button>
                  </Link>
                </CardContent>
              </Card>
            ))}
      ...
      <footer className="border-t border-secondary/20 py-12">
        <div className="container flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-sm text-secondary-foreground/40">© {new Date().getFullYear()} SimchaSync. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
